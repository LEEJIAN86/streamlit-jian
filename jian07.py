...
def  plotting_demo():
    ...
    plt.subplot(222)
    plt.plot(  list( money['A_MONTH'] ), list( money['K_RATE'] ), color='red' , marker='o'     ) 
    plt.xticks(tuple(money['A_MONTH']) )
    plt.title('Korea rate')

    plt.subplot(223)
    plt.plot(  list( money['A_MONTH'] ), list( money['KOSPI'] ), color='black' , marker='o'     ) 
    plt.xticks(tuple(money['A_MONTH']) )
    plt.title('Kospi Rate')

    plt.subplot(224)
    plt.plot(  list( money['A_MONTH'] ), list( money['HOUSE_PRICE'] ), color='orange' , marker='o'     ) 
    plt.xticks(tuple(money['A_MONTH']) )
...
def bar_chart():
    ...
    plt.bar(  x,  y,  color= 'lightslategray' ) 
...
with st.form(key ='Form1'):
    with st.sidebar:
        ...
        select_language = st.sidebar.radio('Data Analysis Results', ('interest rates and house prices Corr', 'Baseball Rankings and Winning Rates Corr', 'else data'))
